...
class TestGetProvider:
    ...
    def test_local_when_available(self):
        with patch("hermes_agent_stt.transcription_tools._HAS_FASTER_WHISPER", True):
            from hermes_agent_stt import _get_provider
    ...
    def test_explicit_local_no_cloud_fallback(self, monkeypatch):
        ...
        with patch("hermes_agent_stt.transcription_tools._HAS_FASTER_WHISPER", False), \
             patch("hermes_agent_stt.transcription_tools._HAS_OPENAI", True), \
             patch("hermes_agent_stt.transcription_tools._has_local_command", return_value=False):
            from hermes_agent_stt import _get_provider
    ...
    def test_local_nothing_available(self, monkeypatch):
        ...
        with patch("hermes_agent_stt.transcription_tools._HAS_FASTER_WHISPER", False), \
             patch("hermes_agent_stt.transcription_tools._HAS_OPENAI", False), \
             patch("hermes_agent_stt.transcription_tools._has_local_command", return_value=False):
            from hermes_agent_stt import _get_provider
            assert _get_provider({"provider": "local"}) == "none"
    ...
    def test_openai_when_key_set(self, monkeypatch):
        monkeypatch.setenv("VOICE_TOOLS_OPENAI_KEY", "sk-test")
        with patch("hermes_agent_stt.transcription_tools._HAS_OPENAI", True):
            from hermes_agent_stt import _get_provider
...
class TestValidateAudioFile:
    ...
    def test_missing_file(self, tmp_path):
        from hermes_agent_stt import _validate_audio_file
    ...
    def test_too_large(self, tmp_path):
        f = tmp_path / "big.ogg"
        f.write_bytes(b"x")
        from hermes_agent_stt import _validate_audio_file, MAX_FILE_SIZE
        real_stat = f.stat()
...
class TestTranscribeLocal:
    ...
    def test_successful_transcription(self, tmp_path):
        ...
        with patch("hermes_agent_stt.transcription_tools._HAS_FASTER_WHISPER", True), \
             ...
             patch("hermes_agent_stt.transcription_tools._local_model", None):
            from hermes_agent_stt import _transcribe_local
...
class TestTranscribeOpenAI:
    ...
    def test_no_key(self, monkeypatch):
        monkeypatch.delenv("VOICE_TOOLS_OPENAI_KEY", raising=False)
        from hermes_agent_stt import _transcribe_openai
...
class TestTranscribeAudio:
    ...
    def test_dispatches_to_local(self, tmp_path):
        ...
        with patch("hermes_agent_stt.transcription_tools._load_stt_config", return_value={"provider": "local"}), \
             patch("hermes_agent_stt.transcription_tools._get_provider", return_value="local"), \
             patch("hermes_agent_stt.transcription_tools._transcribe_local", return_value={"success": True, "transcript": "hi"}) as mock_local:
            from hermes_agent_stt import transcribe_audio
    ...
    def test_dispatches_to_openai(self, tmp_path):
        ...
        with patch("hermes_agent_stt.transcription_tools._load_stt_config", return_value={"provider": "openai"}), \
             patch("hermes_agent_stt.transcription_tools._get_provider", return_value="openai"), \
             patch("hermes_agent_stt.transcription_tools._transcribe_openai", return_value={"success": True, "transcript": "hi"}) as mock_openai:
            from hermes_agent_stt import transcribe_audio
            result = transcribe_audio(str(audio_file))
    ...
    def test_no_provider_returns_error(self, tmp_path):
        ...
        with patch("hermes_agent_stt.transcription_tools._load_stt_config", return_value={}), \
             patch("hermes_agent_stt.transcription_tools._get_provider", return_value="none"):
            from hermes_agent_stt import transcribe_audio
            result = transcribe_audio(str(audio_file))
    ...
    def test_disabled_config_returns_disabled_error(self, tmp_path):
        ...
        with patch("hermes_agent_stt.transcription_tools._load_stt_config", return_value={"enabled": False}), \
             patch("hermes_agent_stt.transcription_tools._get_provider", return_value="none"):
            from hermes_agent_stt import transcribe_audio
            result = transcribe_audio(str(audio_file))
...
class TestNormalizeLocalModel:
    ...
    def test_openai_model_name_maps_to_default(self):
        from hermes_agent_stt import _normalize_local_model, DEFAULT_LOCAL_MODEL
    ...
    def test_valid_local_model_preserved(self):
        from hermes_agent_stt import _normalize_local_model
    ...
    def test_local_transcribe_normalises_model(self):
        ...
        try:
            ...
            with patch("hermes_agent_stt.transcription_tools._HAS_FASTER_WHISPER", True), \
                 patch("hermes_agent_stt.transcription_tools._load_stt_config", return_value={
                     "enabled": True,
                     "provider": "local",
                     "local": {"model": "whisper-1"},
                 }), \
                 patch("hermes_agent_stt.transcription_tools._local_model", None), \
                 patch("hermes_agent_stt.transcription_tools._local_model_name", None), \
