"""Regression: Discord UI view classes must be defined after package install.

When discord.py is NOT installed at module load time, the
``if DISCORD_AVAILABLE:`` guard at the bottom of the discord adapter
...
a successful import so that all view classes are available the moment
...
class TestDefineDiscordViewClasses:
    ...
    def test_registers_all_five_view_classes(self, monkeypatch):
        ...
        dp = importlib.import_module("hermes_agent_discord.adapter")

        # Remove the classes to simulate the state where the module was loaded
        # with DISCORD_AVAILABLE=False (the package-not-installed scenario).
    ...
    def test_check_discord_requirements_calls_define_on_import(self, monkeypatch):
        """check_discord_requirements() must call _define_discord_view_classes() on
        a successful import so view classes exist when DISCORD_AVAILABLE=True."""
        dp = importlib.import_module("hermes_agent_discord.adapter")
        ...
        assert result is True, "check_discord_requirements() should return True after import"
        assert define_called[0], (
            "check_discord_requirements() must call _define_discord_view_classes() "
            "after a successful import so view classes are not undefined"
        )
