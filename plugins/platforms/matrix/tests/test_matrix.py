...
class TestMatrixTypingIndicator:
    ...
    async def test_stop_typing_clears_matrix_typing_state(self):
        ...
        from hermes_agent_matrix import RoomID
...
class TestMatrixRequirements:
    def test_check_requirements_with_token(self, monkeypatch):
        ...
        try:
            ...
            with patch("hermes_agent_matrix.adapter._require_mautrix", side_effect=ImportError("mautrix unavailable")):
    ...
    def test_check_requirements_encryption_true_no_e2ee_deps(self, monkeypatch):
        ...
        import hermes_agent_matrix as matrix_mod
        with patch("hermes_agent_matrix.adapter._check_e2ee_deps", return_value=False):
    ...
    def test_check_e2ee_deps_requires_asyncpg(self, monkeypatch):
        ...
        """
        from hermes_agent_matrix import _check_e2ee_deps
        import builtins
    ...
    @pytest.mark.skip(reason="ensure_and_bind removed — plugin deps are installed by the package manager")
    def test_check_requirements_runs_lazy_install_when_partial(self, monkeypatch):
        """[OBSOLETE] When mautrix was installed but asyncpg/aiosqlite were missing,
        check_matrix_requirements used to run the lazy installer.

        With the plugin package system, all deps are installed up-front by
        the package manager. This test is kept as a marker for #31116 but
        the lazy-install path no longer exists.
        """
        pass
...
class TestMatrixE2EEHardFail:
    ...
    async def test_connect_fails_when_encryption_true_but_no_e2ee_deps(self):
        from hermes_agent_matrix import MatrixAdapter, _check_e2ee_deps
...
class TestJoinedRoomsReference:
    def test_joined_rooms_reference_preserved_after_reassignment(self):
        ...
        from hermes_agent_matrix import _CryptoStateStore
...
class TestMatrixClockSkewWarning:
    ...
    async def test_late_drops_emit_one_shot_clock_skew_warning(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="hermes_agent_matrix.adapter"):
            for i in range(5):
                ev = self._mk_event(
        ...
        skew_warnings = [
            ...
            if r.name == "hermes_agent_matrix.adapter"
...
class TestCreateMatrixSession:
    ...
    async def test_no_proxy_returns_trust_env_session(self):
        with patch.dict("sys.modules", _make_fake_mautrix()):
            from hermes_agent_matrix import _create_matrix_session
