...
def _fake_dingtalk_optional_sdks(monkeypatch):
    ...
    import hermes_agent_dingtalk.adapter as dt
...
class TestDingTalkRequirements:
    ...
    def test_returns_false_when_sdk_missing(self, monkeypatch):
        with patch.dict("sys.modules", {"dingtalk_stream": None}), \
            ...
            monkeypatch.setattr(
                "hermes_agent_dingtalk.adapter.DINGTALK_STREAM_AVAILABLE", False
            )
            from hermes_agent_dingtalk import check_dingtalk_requirements
            assert check_dingtalk_requirements() is False

    def test_returns_false_when_env_vars_missing(self, monkeypatch):
        monkeypatch.setattr(
            "hermes_agent_dingtalk.adapter.DINGTALK_STREAM_AVAILABLE", True
        )
        monkeypatch.setattr("hermes_agent_dingtalk.adapter.HTTPX_AVAILABLE", True)
...
class TestDingTalkAdapterInit:
    ...
    def test_reads_config_from_extra(self):
        from hermes_agent_dingtalk import DingTalkAdapter
...
class TestWebhookDomainAllowlist:
    ...
    def test_api_domain_accepted(self):
        from hermes_agent_dingtalk import _DINGTALK_WEBHOOK_RE
...
class TestExtractMedia:
    ...
    def test_audio_rich_text_item_stays_audio(self):
        ...
        from hermes_agent_dingtalk import DingTalkAdapter, DINGTALK_TYPE_MAPPING
        from gateway.platforms.base import MessageType
...
class TestIncomingHandlerProcess:
    ...
    async def test_process_extracts_session_webhook(self):
        ...
        from hermes_agent_dingtalk import _IncomingHandler, DingTalkAdapter
