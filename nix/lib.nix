...
#
# Dev-shell hooks are split into two layers:
#   passthru.devShellHook  — per-package: stamps ${folder}/package.json,
#                            runs npm install if changed
#   rootDevShellHook       — root-level (once): stamps package-lock.json,
#                            runs npm ci + fix-lockfiles if changed
# devShell.nix runs per-package hooks first, then rootDevShellHook,
# so an npm install that updates the lockfile triggers fix-lockfiles
# in the same shell entry.
{
  pkgs,
  npm-lockfile-fix,
  nodejs,
...
let
  ...
  npmDepsHash = "sha256-a0kbmdV0i9kugSq2znUNqM1qaNsj7tN86zNoRbKXlng=";
...
in
{
  ...
  #   passthru.devShellHook  — stamp-checked npm install (package.json only)
  ...
  mkNpmPassthru =
    ...
    in
    {
      ...
      passthru = {
        # Per-package hook: stamps ${folder}/package.json, runs npm install
        # for that workspace if it changed.  The root-level lockfile hook
        # (rootDevShellHook below) handles npm ci + fix-lockfiles separately.
        devShellHook = pkgs.writeShellScript "npm-dev-hook-${pname}" ''
          REPO_ROOT=$(git rev-parse --show-toplevel)

          _hermes_npm_stamp() {
            sha256sum "${folder}/package.json" \
              2>/dev/null | awk '{print $1}'
          }
          STAMP=".nix-stamps/${pname}"
          STAMP_VALUE="$(_hermes_npm_stamp)"
          if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
            echo "${pname}: package.json changed, running npm install..."
            ( cd "$REPO_ROOT" && ${pkgs.lib.getExe' nodejs "npm"} install -w ${folder} --silent --no-fund --no-audit 2>/dev/null )
            mkdir -p .nix-stamps
  ...
  # Root-level devshell hook: stamps package-lock.json, runs npm ci +
  # fix-lockfiles if changed.  Runs ONCE regardless of how many workspace
  # packages exist.  devShell.nix should run this AFTER the per-package
  # hooks so that npm install (triggered by a package.json change) can
  # update the lockfile first, then this hook picks up the change and
  # runs npm ci + fix-lockfiles in the same shell entry.
  #
  ...
  rootDevShellHook = fixLockfilesExe: pkgs.writeShellScript "npm-root-dev-hook" ''
    REPO_ROOT=$(git rev-parse --show-toplevel)

    STAMP=".nix-stamps/root-lockfile"
    STAMP_VALUE=$(sha256sum "$REPO_ROOT/package-lock.json" 2>/dev/null | awk '{print $1}')
    if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
      echo "npm: package-lock.json changed, running npm ci..."
      ( cd "$REPO_ROOT" && ${pkgs.lib.getExe' nodejs "npm"} ci --silent --no-fund --no-audit 2>/dev/null )
